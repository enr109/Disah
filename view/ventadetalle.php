...
<body>
      ...
      <div class="row mt-5">
        <div class="col-lg-12">
          <table class="table table-dark">
            ...
            <tbody>
              <tr v-for="Ventas in vent">
                <td>{{Ventas.Id}}</td>
                <td>{{Ventas.total}}</td>
                <td>{{Ventas.fecha}}</td>
                <td>{{Ventas.pago}}</td>
                <td>{{Ventas.entrega}}</td>
                <td>{{Ventas.nom}}</td>
                <td>{{Ventas.ap}}</td>
                <td>{{Ventas.am}}</td>
                <td>{{Ventas.cantida}}</td>
                <td>{{Ventas.lugar}}</td>
                <td>{{Ventas.producto}}</td>
                <td>{{Ventas.estado}}</td>
                <td>
                  <div class="btn-group" role="group">
                    <button class="btn btn-secondary" data-toggle="modal" data-target="#editar-productor" @click="pasarDatosEditar(Ventas)" title="Editar"><i class="fas fa-pencil-alt"></i></button>
                  </div>
                </td>
              </tr>
          ...
          <div class="modal-dialog modal-md" role="document">
            <div class="modal-content">
              ...
              <div class="modal-footer">
                <button type="button" class="btn btn-primary" data-dismiss="modal" @click="editarventa()">Actualizar</button>
              </div>
  ...
  ?>
    ...
    <!-- codigo custon-->
    <script type="text/javascript" src="../src/js/muestraVentas.js"></script>
</body>
